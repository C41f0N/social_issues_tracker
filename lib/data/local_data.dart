...
class LocalData with ChangeNotifier {
  ...
  Future<GroupJoinRequest?> addGroupJoinRequest({
    ...
    try {
      ...
      debugPrint('[addGroupJoinRequest] Response status: ${response.statusCode}');
  ...
  Future<void> updateGroupJoinRequestStatus(
    ...
    try {
      ...
      if (newStatus == GroupJoinRequestStatus.cancelled) {
        ...
        debugPrint('[updateGroupJoinRequestStatus] Cancelling request at: $url');
        ...
        debugPrint('[updateGroupJoinRequestStatus] Response status: ${response.statusCode}');
        ...
        if (response.statusCode == 200) {
          ...
          notifyListeners();
          debugPrint('[updateGroupJoinRequestStatus] Request cancelled successfully');
        } else {
          debugPrint('[updateGroupJoinRequestStatus] Error: ${response.statusCode}');
        }
        ...
        if (response.statusCode == 200) {
          ...
          notifyListeners();
          debugPrint('[updateGroupJoinRequestStatus] Request updated successfully');
        } else {
          debugPrint('[updateGroupJoinRequestStatus] Error: ${response.statusCode}');
        }
      }
  ...
  Future<void> fetchGroupJoinRequests({String? direction}) async {
    ...
    try {
      ...
      debugPrint('[fetchGroupJoinRequests] Response status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        for (final requestData in requestsList) {
          ...
          if (requestData['issue_title'] != null) {
            ...
            final issueOwnerUsername = requestData['issue_owner_username'] as String?;
            
            // Ensure issue owner user exists
            final existingUserIndex = storedUsers.indexWhere((u) => u.id == issueOwnerId);
            if (existingUserIndex == -1) {
              final user = models.User(
                id: issueOwnerId,
                name: issueOwnerUsername ?? 'Unknown',
                imageUrl: 'https://api.dicebear.com/9.x/pixel-art/png?seed=${Uri.encodeComponent(issueOwnerUsername ?? issueOwnerId)}',
              );
          ...
          if (requestData['group_name'] != null) {
            ...
            final groupOwnerUsername = requestData['group_owner_username'] as String?;
            
            // Ensure group owner user exists
            final existingUserIndex = storedUsers.indexWhere((u) => u.id == groupOwnerId);
            if (existingUserIndex == -1) {
              final user = models.User(
                id: groupOwnerId,
                name: groupOwnerUsername ?? 'Unknown',
                imageUrl: 'https://api.dicebear.com/9.x/pixel-art/png?seed=${Uri.encodeComponent(groupOwnerUsername ?? groupOwnerId)}',
              );
              storedUsers.add(user);
            }
          }
        ...
        notifyListeners();
        debugPrint('[fetchGroupJoinRequests] Loaded ${requestsList.length} requests');
      } else {
