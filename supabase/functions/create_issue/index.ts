...
serve(async (req) => {
  try {
    ...
    const displayPicture = form.get("display_picture");
    ...
    const { data: issueId, error: createError } = await supabase.rpc("create_issue", {
      p_title: title.toString(),
      p_description: description.toString(),
      p_user_id: userId,
      p_display_picture_path: null,
      p_group_id: null
    });
    if (createError || !issueId) {
      return new Response(JSON.stringify({
        error: createError?.message || "Failed to create issue"
      }), {
        status: 400
      });
    }
    ...
    try {
      ...
      if (displayPicture && displayPicture.size > 0) {
        ...
        const { error: uploadError } = await supabase.storage.from(bucket).upload(storagePath, displayPicture, {
          cacheControl: "3600",
          upsert: false
        });
        if (uploadError) {
        ...
        const { error: updateError } = await supabase.from("issues").update({
          display_picture_path: displayPicturePath
        }).eq("issue_id", issueId);
        if (updateError) {
      ...
      for (const attachment of attachments) {
        ...
        const fileName = ext ? `${uuid}.${ext}` : uuid;
        const storagePath = `${issueId}/${fileName}`;
        const { error: uploadError } = await supabase.storage.from(bucket).upload(storagePath, attachment, {
          cacheControl: "3600",
          upsert: false
        });
        if (uploadError) {
        ...
        const { data: attachmentId, error: rpcError } = await supabase.rpc("add_post_attachment", {
          p_issue_id: issueId,
        ...
        const { data: signedData, error: signedErr } = await supabase.storage.from(bucket).createSignedUrl(storagePath, 60 * 60 * 24 * 30);
        if (!signedErr && signedData) {
      ...
      if (uploadedFiles.length > 0) {
        await supabase.storage.from(bucket).remove(uploadedFiles).catch(() => { });
      }
      // Delete the issue if we created it
      await supabase.from("issues").delete().eq("issue_id", issueId).catch(() => { });
      return new Response(JSON.stringify({
